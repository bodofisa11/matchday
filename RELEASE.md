...
- **Consistent page-top spacing.** Team pages no longer have an oversized gap above the breadcrumb — every page now spaces its breadcrumb the same way under the top bar.
...
- **World Cup** views now resolve team names correctly. National teams (Argentina, Brazil, …) live in the database's `fb_nations` table rather than `fb_clubs`, and the backend now stores World Cup fixtures, group standings, scorers, and squads against the nation instead of a club. The Supabase query layer reads both the club and the nation for each slot and shows whichever applies, so WC fixtures, group tables, top scorers, the teams list, and squads display proper country names. Domestic-league and Champions League views are unchanged.
